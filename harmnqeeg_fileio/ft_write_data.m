...
switch dataformat
  ...
  case 'empty'
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [numC, numS] = size(dat);
    fprintf('Pretending to write %i samples from %i channels...\n',numS,numC);
  ...
  case 'fcdc_global'
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ~isempty(evt)
      error('writing events is not supported here, please see FT_WRITE_EVENT');
    end
  ...
  case 'fcdc_buffer'
    ...
    if ~isempty(dat)
      ...
      if size(dat,2)>MAXNUMSAMPLE
        error('number of samples exceeds the size of the ring buffer');
      end
  ...
  case 'neuralynx_sdma'
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ~isempty(evt)
      error('writing events is not supported');
    end
  ...
  case {'flac' 'm4a' 'mp4' 'oga' 'ogg' 'wav'}
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if append
      error('appending data is not supported for this data format');
    end
    if ~isempty(evt)
      error('writing events is not supported');
    end
  ...
  case 'anywave_ades'
    ...
    for i=1:numel(dattype)
      ...
      if ~all(strcmp(unit, unit{1}))
        error('channels of the same type with different units are not supported');
      end
      datunit{i} = unit{1};
    end
  ...
  case 'snirf'
    ...
    warning('assuming that the nominal wavelengths are %d and %d nm', WL1.nominal, WL2.nominal)
    ...
    warning('assuming that the input data represents (change in) optical densities')
