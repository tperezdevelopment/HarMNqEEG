...
if nargin==1
  ...
  if EDF.Cal(tmp)~=1
    ...
    warning('FieldTrip:BDFCalibration', 'calibration for status channel appears incorrect, setting it to 1', timeout);
    EDF.Cal(tmp) = 1;
  end
  if EDF.Off(tmp)~=0
    timeout = 60*15; % do not show it for the next 15 minutes
    warning('FieldTrip:BDFOffset', 'offset for status channel appears incorrect, setting it to 0', timeout);
    EDF.Off(tmp) = 0;
  end
