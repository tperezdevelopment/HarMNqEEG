function s = HarMNqEEG_stats_skewness(x,flag,dim)
...
x0 = x - mean(x,dim, 'omitnan'); %% Changed by tperezdevelopment from nanmean
s2 = mean(x0.^2,dim, 'omitnan'); % this is the biased variance estimator; %% Changed by tperezdevelopment from nanmean
m3 = mean(x0.^3,dim, 'omitnan'); %% Changed by tperezdevelopment from nanmean
s = m3 ./ s2.^(1.5);
