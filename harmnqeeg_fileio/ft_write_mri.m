...
elseif ft_datatype(dat, 'volume')
  ...
  if ~isempty(transform) && ~isequal(transform, mri.transform)
    error('the transformation matrices are not consistent');
  end
  if ~isempty(unit)
...
switch dataformat
  ...
  case {'analyze_old'}
    ...
    switch class(dat)
      ...
      otherwise
        error('unsupported datatype "%s" to write to analyze_old format', class(dat));
    end
  ...
  case {'nifti' 'nifti_gz' 'nifti2'}
    ...
    switch(datatype)
      ...
      case 'uint8'
        datatype = 'uchar';
      otherwise
        error('unsupported datatype "s" to write to nifti format', datatype);
    end
