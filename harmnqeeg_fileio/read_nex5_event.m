...
if isempty(adindx)  % this would otherwise produce an error
  warning('No continuous variables found - using hdr.FileHeader.Frequency');
...
for mrkn = 1:numel(mrkvarnum)
  ...
  if isempty(ts)
    warning(['skipping marker ' name ' because no timestamps were found'])
    continue
  end
...
for intVarIndex = 1:numel(intvarnum)
  ...
  if isempty(intStarts) && isempty(intEnds)
    warning(['skipping interval ' deblank(vh.Name) ' because no timestamps were found'])
    continue
  end
...
for ev = 1:numel(evtvarnum)
  ...
  if isempty(ts)
    warning(['skipping event ' deblank(vh.Name) ' because no timestamps were found'])
    continue
  end
