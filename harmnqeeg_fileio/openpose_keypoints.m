...
if (lastframe-firstframe+1)~=numframes
  warning('there are missing frames, the timeseries might be discontinuous');
end
...
framerate = 25;
warning('assuming a default framerate of 25 Hz');
...
elseif needdat
  fprintf('reading %d frames', numframes);
  % read all files
...
elseif needevt
  warning('cannot read events from %s', filename);
