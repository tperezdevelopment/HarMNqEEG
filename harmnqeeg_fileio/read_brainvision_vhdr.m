...
if ~isempty(vhdr.NumberOfChannels)
  ...
  warning once FieldTrip:fileio:UnknownResolution
  ...
  for i=1:vhdr.NumberOfChannels
    ...
    if ~isempty(resolution)
      ...
      warning('FieldTrip:fileio:UnknownResolution', 'unknown resolution (i.e. recording units) for channel %d in "%s"', i, filename);
      vhdr.resolution(i) = 1;
    end
  end
  
  % switch the warning state back to the default
  warning on FieldTrip:fileio:UnknownResolution
end
...
if isempty(info)
  ...
  if ~isempty(info)
    ...
    error('cannot determine the location of the data file "%s"', dataFile);
  end
else
...
if ~isempty(markerFile)
  ...
  if isempty(info)
    ...
    if ~isempty(info)
      ...
      error('cannot determine the location of the marker file "%s"', markerFile);
    end
  else
