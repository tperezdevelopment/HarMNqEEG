...
if isfield(object, 'tra') && isfield(object, 'chanunit')
  ...
  for i=1:length(object.chanunit)
    ...
    elseif strcmp(object.chanunit{i}, 'snr')
      ...
      error('unexpected units %s', object.chanunit{i});
    end
  end % for
end % if
...
if isfield(object, 'initial') && ~strcmp(target, 'mm')
  ...
  warning('Removing field "initial" because potential transformations of normalised volumes only work if geometrical values are expressed in "mm"');
end

if isfield(object, 'params') && ~strcmp(target, 'mm')
  warning('Removing field "params" because potential transformations of normalised volumes only work if geometrical values are expressed in "mm"');
