function [x] = HarMNqEEG_ft_struct2char(x, maxdepth)
% From FieldTrip, Module Fileio. Url: https://www.fieldtriptoolbox.org/
% Modified Tool Author: Eng. Tania Perez Ramirez <tperezdevelopment@gmail.com>.
% Cuban Center for Neurosciences
% Release notes: this will delete the fieldtrip dependecies and reduce
% code
...
switch class(a)
  case 'struct'
    ...
    for j=1:length(a(:))
      ...
      for i=1:length(fna)
        fn = fna{i};
        ra = getfield(a(j), fn); %#ok<GFLD> 
        ra = convert(ra, depth+1, maxdepth);
        a(j) = setfield(a(j), fn, ra); %#ok<SFLD> 
      end
    end
