...
if iscell(filename)
  ...
  warning('concatenating events from %d files', numel(filename));
...
if ~isempty(trigindx)
  warning('please use ''chanindx'' instead of ''trigindx''')
  chanindx = trigindx;
end
...
if ~isempty(triglabel)
  error('please use ''chanindx'' instead of ''triglabel''')
...
switch eventformat
  ...
  case 'neuroomega_mat'
    ...
    if ~ismember(detectflank,{'up','down','both'})
      error('incorrect specification of detectflank. Use up, down or both');
    end
