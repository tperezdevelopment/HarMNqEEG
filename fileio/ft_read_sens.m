  ...
  if isbids
    ...
    if ~isempty(tsvfile) && (isempty(senstype) || strcmp(senstype, 'eeg'))
      ...
      if exist(jsonfile, 'file')
        ...
        warning('the content of the electrodes.json is not used')
      ...
      if exist(coordsysfile, 'file')
        ...
        warning('the content of the coordsystem.json is not used')
        % FIXME do something with the content
      end
    end
  end
...
switch fileformat
  ...
  case {'ctf_ds', 'ctf_res4', 'ctf_old', 'neuromag_fif', 'neuromag_mne', '4d', '4d_pdf', '4d_m4d', '4d_xyz', 'yokogawa_ave', 'yokogawa_con', 'yokogawa_raw', 'ricoh_ave', 'ricoh_con', 'itab_raw' 'itab_mhd', 'netmeg'}
    ...
    elseif isfield(hdr, 'elec')
      ...
      error('there is no electrode nor gradiometer information present in the header');
    end
  ...
  case {'homer_nirs', 'snirf', 'artinis_oxy3', 'artinis_oxy4', 'artinis_oxy5', 'artinis_oxyproj', 'nirx_wl1', 'nirx_wl2', 'nirx_tpl'}
    ...
    if isfield(hdr, 'opto')
      ...
      error('there is no optode information present in the header');
    end
  ...
  case 'besa_elp'
    ...
    switch format
      ...
      case 5
        ...
        radius    = tmp{5};
      otherwise
        error('unsupported file format for .elp');
    end
    fclose(fid);
  ...
  case 'neuromag_iso'
    ...
    warning('creating fake channel names for neuromag_iso');
