...
if ~isempty(sel)
  warning('the option ''hastrialdef'' is deprecated, please use ''hassampleinfo'' instead');
  varargin{2*sel-1} = 'hassampleinfo';
end
sel = find(strcmp(varargin(1:2:end), 'inside'));
if ~isempty(sel)
  warning('the option ''inside'' is deprecated, please use ''insidestyle'' instead');
  varargin{2*sel-1} = 'insidestyle';
end
sel = find(strcmp(varargin(1:2:end), 'cmbrepresentation'));
if ~isempty(sel)
  warning('the option ''cmbrepresentation'' is deprecated, please use ''cmbstyle'' instead');
...
if ~isequal(feedback, 'no') % can be 'yes' or 'text'
  ...
  if ismesh
    ...
    elseif isfield(data,'tet')
      ...
      fprintf('the input is mesh data with %d vertices', npos);
    end
  end % if ismesh
  ...
  elseif isvolume
    if issegmentation
      fprintf('the input is segmented volume data with dimensions [%d %d %d]\n', data.dim(1), data.dim(2), data.dim(3));
...
if ~isempty(dtype)
  ...
  if ~okflag
    ...
    if ~isempty(helpfun)
      ...
      error('This function requires %s data as input, see %s.', typestr, helpstr);
    else
      error('This function requires %s data as input.', typestr);
    end
  end % if okflag
end
...
if ~isempty(dimord)
  ...
  if ~okflag
    % construct an error message
    error('This function requires data with a dimord of %s.', printor(dimord, true));
  end % if okflag
end
...
if ~isempty(stype)
  ...
  if ~okflag
    % construct an error message
    error('This function requires data with an %s sensor array.', printor(stype, true));
  end % if okflag
end
...
if ~isempty(iseeg)
  ...
  if ~okflag && isequal(iseeg, 'yes')
    error('This function requires EEG data with an ''elec'' field');
  elseif ~okflag && isequal(iseeg, 'no')
    error('This function should not be given EEG data with an ''elec'' field');
  end % if okflag
end
...
if all(indexed)
  ...
  for k = 1:numel(fn)
    fprintf('the volume of each of the segmented compartments in "%s" is', fn{k});
    ...
    for m = 1:numel(tissuelabel)
      ...
      fprintf('%s : %8.0f %s (%6.2f %%)', pad(tissuelabel{m}, width), volume, voxelunit, 100*volume/totalvolume);
    end
    % print the summary of the totals
    fprintf('%s : %8.0f %s (%6.2f %%)', pad('total segmented', width), summedvolume, voxelunit, 100*summedvolume/totalvolume);
    fprintf('%s : %8.0f %s (%6.2f %%)', pad('total volume',    width), totalvolume, voxelunit, 100*totalvolume/totalvolume);
...
elseif all(probabilistic)
  ...
  totalvolume  = prod(segmentation.dim)*voxelvolume;
  summedvolume = 0;
  fprintf('the volume of each of the segmented compartments is');
  for k = 1:numel(fn)
    tissuelabel = fn{k};
    tissueprobability = segmentation.(tissuelabel);
    volume = sum(tissueprobability(:)*voxelvolume);
    summedvolume = summedvolume + volume;
    fprintf('%s : %8.0f %s (%6.2f %%)', pad(tissuelabel, width), volume, voxelunit, 100*volume/totalvolume);
  end
  % print the summary of the totals
  fprintf('%s : %8.0f %s (%6.2f %%)', pad('total segmented', width), summedvolume, voxelunit, 100*summedvolume/totalvolume);
  fprintf('%s : %8.0f %s (%6.2f %%)', pad('total volume',    width), totalvolume, voxelunit, 100*totalvolume/totalvolume);
