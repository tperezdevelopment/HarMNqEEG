...
switch fileformat
  ...
  case {'ricoh_mrk', 'ricoh_ave', 'ricoh_con'}
    ...
    if any([mrk_pnt(:).meg_pos])
      ...
      error('No coil information found in the file');
    end
    
    %% Digitized points
    if ~isexported
      fprintf('The input file is an original one: only marker-coil positions are loaded\n');
      ...
      fprintf('The input file is a third-party-exported one including the digitized points\n');
  ...
  case {'yokogawa_mrk', 'yokogawa_ave', 'yokogawa_con'}
    if ft_hastoolbox('yokogawa_meg_reader')
      ...
      if ~any([mrk_pnt(:).meg_pos])
        fprintf('Reading marker-coil positions from a .mrk file\n');
        [p, f, x] = fileparts(filename);
