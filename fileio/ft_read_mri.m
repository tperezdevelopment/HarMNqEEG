  ...
  case {'afni_brik' 'afni_head'}
    ...
    if contains(filename, 'TTatlas') || (isfield(hdr, 'TEMPLATE_SPACE') && ~isempty(hdr.TEMPLATE_SPACE))
      ...
      elseif startsWith(space, 'tlrc')
        ...
        warning('space ''tlrc'' might be ambiguous, here assuming the coordsys to be ''tal''');
        space = 'tal';
      end
      if ismember(space, {'tal' 'mni'})
        % xyz orientation should be RAS
        if ~strcmp(coordsys, 'ras')
          warning('the template space suggests that the image is in %s coordinates, but the xyz orientation %s does not match this', space, coordsys);
          xxx2ras = true;
        else
          coordsys = space;
        end
    ...
    if ~isfixel
      ...
      switch fix2vox_fun
        ...
        case 'none'
          tmpdata = tmpdata.';
        otherwise
          error('unsupported fixel2voxel operation requested');
      end
